...
export const hospitalPaymentModes: TransactionPaymentMode[] = [
  "Cash",
  "UPI",
  "Bank Transfer",
  "Cheque",
...
export const createHospitalTransactionRecords = ({
  prefix,
  categories,
  descriptions,
  amountOffset,
}: CreateHospitalRecordsOptions): TransactionRecord[] =>
  Array.from({ length: 50 }, (_, index) => {
    const contact =
      hospitalTransactionContacts[index % hospitalTransactionContacts.length];
    ...
    return {
      ...
      paymentMode: hospitalPaymentModes[index % hospitalPaymentModes.length],
      description: descriptions[index % descriptions.length],
      amount: 1250 + ((index * 1375) % 48500) + amountOffset,
    };
  });
