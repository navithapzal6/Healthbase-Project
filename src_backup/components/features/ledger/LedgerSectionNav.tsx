...
const LedgerSectionNav = ({
  ...
  return (
    <CollapsiblePanel
      ...
      className="p-2"
    >
      <div className={`mb-3 flex h-9 items-center px-2 ${collapsed ? "invisible" : ""}`}>
        <p className="whitespace-nowrap text-sm font-semibold text-slate-900">
          Ledger Menu
        </p>
      </div>

      <nav className="space-y-2" aria-label="Ledger sections">
        {ledgerSections.map((section) => {
          ...
          return (
            <button
              key={section.id}
              type="button"
              onClick={() => onChange(section.id as LedgerSectionId)}
