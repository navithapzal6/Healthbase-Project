        ...
        <table className="w-full min-w-[760px] table-fixed border-collapse text-left text-xs">
          <thead className="sticky top-0 z-20 bg-slate-50 text-xs font-semibold text-slate-600">
            <tr className="border-b border-slate-200">
              <th className="sticky left-0 z-30 w-12 bg-slate-50 text-center shadow-[8px_0_12px_-12px_rgba(15,23,42,0.35)]">
                <ListCheckbox
              ...
              <th>{nameColumn}</th>
              <th className="w-40 text-center">{assignedColumn}</th>
              <th className="w-40 text-center">{unassignedColumn}</th>
              <th className="sticky right-0 z-30 w-20 bg-slate-50 text-right shadow-[-8px_0_12px_-12px_rgba(15,23,42,0.35)]">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="text-slate-600">
            {pageRecords.map((record, index) => {
              ...
              return (
                <tr
                  key={record.id}
                  className={`group h-11 border-b border-slate-100 transition-colors last:border-0 ${
                    selected
                      ? "bg-primary/[0.04]"
                ...
                >
                  <td
                    className={`sticky left-0 z-10 text-center shadow-[8px_0_12px_-12px_rgba(15,23,42,0.35)] ${
                      selected
                        ? "bg-primary-surface"
                  ...
                  <td
                    className={`sticky right-0 z-10 shadow-[-8px_0_12px_-12px_rgba(15,23,42,0.35)] ${
                      selected
                        ? "bg-primary-surface"
                        : "bg-white group-hover:bg-[#fcfcff]"
                    }`}
        ...
        {pageRecords.length === 0 && (
          <div className="flex min-h-48 items-center justify-center text-center">
            <p className="text-sm text-slate-500">
              No {section.label.toLowerCase()} records found.
            </p>
          </div>
        )}
      </ListTable>
