...
const PaymentWorkspace = ({ module = "payment" }: PaymentWorkspaceProps) => {
  ...
  const contactHistory = useMemo(() => {
    const history = records.filter(
      (record) => record.contactId === selectedContactId,
    );
    ...
    return [...history].sort((first, second) => {
      const result =
        historySortValue === "amount"
          ...
          : String(
              first[historySortValue as keyof PaymentRecord],
            ).localeCompare(
              String(second[historySortValue as keyof PaymentRecord]),
            );

      return historySortDirection === "asc" ? result : -result;
    });
  }, [historySortDirection, historySortValue, records, selectedContactId]);
