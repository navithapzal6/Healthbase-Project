...
const expandRows = (
  rows: Array<[name: string, description: string, createdAt: string]>,
  count = 50,
): Array<[name: string, description: string, createdAt: string]> =>
  Array.from({ length: count }, (_, index) => {
    const [name, description, createdAt] = rows[index % rows.length];
    const batch = Math.floor(index / rows.length) + 1;

    return batch === 1
      ? [name, description, createdAt]
      : [`${name} ${batch}`, `${description} · Record ${batch}`, createdAt];
  });

export const initialLedgerRecords: Record<LedgerSectionId, LedgerRecord[]> = {
  unit: createRecords("unit", expandRows([
  ...
  expense: createRecords("expense", expandRows([
  ...
  bank: createRecords("bank", expandRows([
